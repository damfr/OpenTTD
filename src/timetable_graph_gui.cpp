...
protected:
	...
	SmallVector<int, 8> yindexPositions; //Positions of each row
	uint rowCount;
	...
	struct GraphPoint {
		...
		GraphPoint(const Order* order, uint yindex, GraphPoint* next = NULL) : order(order), next(next), yindex(yindex) {}
		~GraphPoint() {
			delete next;
		}
	};
	...
	SmallVector<DestinationID, 8> destinations; //The destinations (rows of the graph)
...
public:
	...
	TimetableGraphWindow(WindowDesc *desc, WindowNumber window_number) : Window(desc), orderList(NULL),
			vli(VehicleListIdentifier::UnPack(window_number)),
			YlabelWidth(0), mainGraphLine(NULL)
	{
...
protected:
	...
	virtual void UpdateWidgetSize(int widget, Dimension *size, const Dimension &padding, Dimension *fill, Dimension *resize)
	{
		...
		Dimension dmin;
		dmin.height = yindexPositions.Length() * FONT_HEIGHT_SMALL;
