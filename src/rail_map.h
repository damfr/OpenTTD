 ...
 */
static inline RailTileType GetRailTileType(ExtendedTileIndex t)
{
	//assert(IsTileType(t, MP_RAILWAY));
	return (RailTileType)GB(GetElevatedTile(t).m5, 6, 2);
}
 ...
 */
static inline bool IsPlainRail(ExtendedTileIndex t)
{
 ...
 * @return true if and only if the tile is normal rail (with or without signals)
 */
static inline bool IsPlainRailTile(ExtendedTileIndex t)
{
 ...
 */
static inline bool HasSignals(ExtendedTileIndex t)
{
 ...
 */
static inline void SetHasSignals(ExtendedTileIndex tile, bool signals)
{
	assert(IsPlainRailTile(tile));
	SB(GetElevatedTile(tile).m5, 6, 1, signals);
}
 ...
 */
static inline bool IsRailDepot(ExtendedTileIndex t)
{
 ...
 * @return true if and only if the tile is a rail depot
 */
static inline bool IsRailDepotTile(ExtendedTileIndex t)
{
 ...
 */
static inline RailType GetRailType(ExtendedTileIndex t)
{
	return (RailType)GB(GetElevatedTileExt(t).m8, 0, 6);
}
 ...
 */
static inline void SetRailType(ExtendedTileIndex t, RailType r)
{
	SB(GetElevatedTileExt(t).m8, 0, 6, r);
}
 ...
 */
static inline TrackBits GetTrackBits(ExtendedTileIndex tile)
{
	assert(IsPlainRailTile(tile));
	return (TrackBits)GB(GetElevatedTile(tile).m5, 0, 6);
}
 ...
 */
static inline void SetTrackBits(ExtendedTileIndex t, TrackBits b)
{
	assert(IsPlainRailTile(t));
	SB(GetElevatedTile(t).m5, 0, 6, b);
}
 ...
 */
static inline bool HasTrack(ExtendedTileIndex tile, Track track)
{
 ...
 */
static inline DiagDirection GetRailDepotDirection(ExtendedTileIndex t)
{
 ...
 */
static inline Track GetRailDepotTrack(ExtendedTileIndex t)
{
 ...
 */
static inline TrackBits GetRailReservationTrackBits(ExtendedTileIndex t)
{
	assert(IsPlainRailTile(t));
	byte track_b = GB(GetElevatedTile(t).m2, 8, 3);
	Track track = (Track)(track_b - 1);    // map array saves Track+1
	if (track_b == 0) return TRACK_BIT_NONE;
	return (TrackBits)(TrackToTrackBits(track) | (HasBit(GetElevatedTile(t).m2, 11) ? TrackToTrackBits(TrackToOppositeTrack(track)) : 0));
}
 ...
 */
static inline void SetTrackReservation(ExtendedTileIndex t, TrackBits b)
{
	assert(IsPlainRailTile(t));
	assert(b != INVALID_TRACK_BIT);
	assert(!TracksOverlap(b));
	Track track = RemoveFirstTrack(&b);
	SB(GetElevatedTile(t).m2, 8, 3, track == INVALID_TRACK ? 0 : track + 1);
	SB(GetElevatedTile(t).m2, 11, 1, (byte)(b != TRACK_BIT_NONE));
}
 ...
 */
static inline bool TryReserveTrack(ExtendedTileIndex tile, Track t)
{
 ...
 */
static inline void UnreserveTrack(ExtendedTileIndex tile, Track t)
{
	assert(HasTrack(tile, t));
	TrackBits res = GetRailReservationTrackBits(tile);
 ...
 */
static inline bool HasDepotReservation(ExtendedTileIndex t)
{
 ...
 */
static inline void SetDepotReservation(ExtendedTileIndex t, bool b)
{
	assert(IsRailDepot(t));
	SB(GetElevatedTile(t).m5, 4, 1, (byte)b);
}
 ...
 */
static inline TrackBits GetDepotReservationTrackBits(ExtendedTileIndex t)
{
...
static inline SignalType GetSignalType(ExtendedTileIndex t, Track track)
{
	assert(GetRailTileType(t) == RAIL_TILE_SIGNALS);
	byte pos = (track == TRACK_LOWER || track == TRACK_RIGHT) ? 4 : 0;
	return (SignalType)GB(GetElevatedTile(t).m2, pos, 3);
}

static inline void SetSignalType(ExtendedTileIndex t, Track track, SignalType s)
{
	assert(GetRailTileType(t) == RAIL_TILE_SIGNALS);
	byte pos = (track == TRACK_LOWER || track == TRACK_RIGHT) ? 4 : 0;
	SB(GetElevatedTile(t).m2, pos, 3, s);
	if (track == INVALID_TRACK) SB(GetElevatedTile(t).m2, 4, 3, s);
}

static inline bool IsPresignalEntry(ExtendedTileIndex t, Track track)
{
	return GetSignalType(t, track) == SIGTYPE_ENTRY || GetSignalType(t, track) == SIGTYPE_COMBO;
}

static inline bool IsPresignalExit(ExtendedTileIndex t, Track track)
{
	return GetSignalType(t, track) == SIGTYPE_EXIT || GetSignalType(t, track) == SIGTYPE_COMBO;
}

/** One-way signals can't be passed the 'wrong' way. */
static inline bool IsOnewaySignal(ExtendedTileIndex t, Track track)
{
	return GetSignalType(t, track) != SIGTYPE_PBS;
}

static inline void CycleSignalSide(ExtendedTileIndex t, Track track)
{
	byte sig;
	byte pos = (track == TRACK_LOWER || track == TRACK_RIGHT) ? 4 : 6;

	sig = GB(GetElevatedTile(t).m3, pos, 2);
	if (--sig == 0) sig = IsPbsSignal(GetSignalType(t, track)) ? 2 : 3;
	SB(GetElevatedTile(t).m3, pos, 2, sig);
}

static inline SignalVariant GetSignalVariant(ExtendedTileIndex t, Track track)
{
	byte pos = (track == TRACK_LOWER || track == TRACK_RIGHT) ? 7 : 3;
	return (SignalVariant)GB(GetElevatedTile(t).m2, pos, 1);
}

static inline void SetSignalVariant(ExtendedTileIndex t, Track track, SignalVariant v)
{
	byte pos = (track == TRACK_LOWER || track == TRACK_RIGHT) ? 7 : 3;
	SB(GetElevatedTile(t).m2, pos, 1, v);
	if (track == INVALID_TRACK) SB(GetElevatedTile(t).m2, 7, 1, v);
}
 ...
 */
static inline void SetSignalStates(ExtendedTileIndex tile, uint state)
{
	SB(GetElevatedTile(tile).m4, 4, 4, state);
}
 ...
 */
static inline uint GetSignalStates(ExtendedTileIndex tile)
{
	return GB(GetElevatedTile(tile).m4, 4, 4);
}
 ...
 */
static inline SignalState GetSingleSignalState(ExtendedTileIndex t, byte signalbit)
{
 ...
 */
static inline void SetPresentSignals(ExtendedTileIndex tile, uint signals)
{
	SB(GetElevatedTile(tile).m3, 4, 4, signals);
}
 ...
 */
static inline uint GetPresentSignals(ExtendedTileIndex tile)
{
	return GB(GetElevatedTile(tile).m3, 4, 4);
}
 ...
 */
static inline bool IsSignalPresent(ExtendedTileIndex t, byte signalbit)
{
 ...
 * given rail tile.
 */
static inline bool HasSignalOnTrack(ExtendedTileIndex tile, Track track)
{
 ...
 */
static inline bool HasSignalOnTrackdir(ExtendedTileIndex tile, Trackdir trackdir)
{
 ...
 * the signal that is facing us (for which we stop when it's red).
 */
static inline SignalState GetSignalStateByTrackdir(ExtendedTileIndex tile, Trackdir trackdir)
{
	assert(IsValidTrackdir(trackdir));
 ...
 */
static inline void SetSignalStateByTrackdir(ExtendedTileIndex tile, Trackdir trackdir, SignalState state)
{
 ...
 */
static inline bool HasPbsSignalOnTrackdir(ExtendedTileIndex tile, Trackdir td)
{
 ...
 * @param td the trackdir to check
 */
static inline bool HasOnewaySignalBlockingTrackdir(ExtendedTileIndex tile, Trackdir td)
{
...
static inline void MakeRailNormal(ExtendedTileIndex t, Owner o, TrackBits b, RailType r)
{
	SetTileType(t, MP_RAILWAY);
	SetTileOwner(t.index, o);
	SetTileOwner(t, o); //For now, owner of ground tile is same as owner of elevated tile

	if (t.height == 0) //TODO boats
		SetDockingTile(t.index, false); 
	else {
		//Set elevated track above value
		SetBit(_m[t.index].type, 2);
		SetBit(_m[t.index].type, 3);

		//set track height
		//GetElevatedTile(t).height = t.height;
	}
	GetElevatedTile(t).m2 = 0;
	GetElevatedTile(t).m3 = 0;
	GetElevatedTile(t).m4 = 0;
	GetElevatedTile(t).m5 = RAIL_TILE_NORMAL << 6 | b;
	SB(GetElevatedTileExt(t).m6, 2, 4, 0);
	GetElevatedTileExt(t).m7 = 0;
	GetElevatedTileExt(t).m8 = r;
}
