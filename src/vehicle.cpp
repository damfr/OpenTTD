		...
		if (prev_order != NULL) {
			if (!start_autofill && prev_order->HasDeparture()) {
				...
				if (v->cur_real_order_index == v->autofill_start_order_index && order->HasArrival()) {
					...
					int travel_time = std::max(SubtractFromDate(_date, vehicle_offset) - prev_order->GetDeparture(), 0);
					...
					int32 travel_time = std::max(SubtractFromDate(_date, vehicle_offset) - prev_departure - v->lateness_counter, 0);
