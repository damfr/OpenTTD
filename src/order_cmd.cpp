...
TileIndex Order::GetLocation(const Vehicle *v, bool airport) const
{
	switch (this->GetType()) {
		...
		case OT_GOTO_DEPOT:
			...
			return (v->type == VEH_AIRCRAFT) ? Station::Get(this->GetDestination())->xy : Depot::Get(this->GetDestination())->xy.index;
		...
		case OT_GOTO_DEPOT:
			...
			if (v->current_order.GetDepotActionType() & ODATFB_NEAREST_DEPOT) {
				...
				if (v->FindClosestDepot(&location, &destination, &reverse)) {
					...
					if (v->type == VEH_TRAIN && reverse) DoCommand(v->tile.index, v->index, 0, DC_EXEC, CMD_REVERSE_TRAIN_DIRECTION);
				...
				if (v->type != VEH_AIRCRAFT) {
					v->SetDestTile(Depot::Get(order->GetDestination())->xy.index);
				} else {
					Aircraft *a = Aircraft::From(v);
